...
export default ({ data }) => {
  ...
  const playls = feeds.map(e => ({
    url: e.node.enclosure.url,
    title: e.node.title,
  }))
  const somas = [
    {
      ...
      >
        ...
        >
          ...
          <table>
            ...
            <tbody>
              {feeds.map(({ node }, index) => (
                <tr key={index}>
                  <td style={{ color: "#496984" }}>
                    {node.pubDate.slice(0, 11)}
                  </td>
                  <td>
                    <a
                      href={node.link}
                      style={{ textDecoration: "none", color: "#f0eee7" }}
                    >
                      {node.title}
                    </a>
                  </td>
